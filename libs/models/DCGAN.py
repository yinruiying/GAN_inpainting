...
from libs.datas.poisson_editing import poisson_edit
...
class DCGAN(object):
    ...
    def save_image(self, tensor, target, mask, save_path):
        ...
        # tmp = np.hstack((sample, target, res))
        # cv2.imshow('', tmp)
        # cv2.waitKey()
    ...
    def complete(self, cfgs=None):
        ...
        source_imagedir = os.path.join(self.output_dir, self.dataset, self.model_name, 'results_mafa_random',  "source_images")
        masked_imagedir = os.path.join(self.output_dir, self.dataset, self.model_name, 'results_mafa_random', "masked_images")
        # gen_imagedir = os.path.join(self.output_dir, self.dataset, self.model_name, 'results', "gen_images")
        impainted_imagedir = os.path.join(self.output_dir, self.dataset, self.model_name, 'results_of_demo', "inpainted_images")
        ...
        for i, image in enumerate(images):
            target = cv2.imread(aligned_images[i])  # poisson编辑的target
            mask = 255 - cv2.imread(mask_images[i], cv2.IMREAD_GRAYSCALE)  # poisson编辑的mask
